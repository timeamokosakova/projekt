   ...
   <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">  
            ...
            <p>
                <div style="position: relative; top: -30px; left: 100px">
                    <b>Ulica: </b>  <input type="text" name="ulica"  value="<?php echo $ulica;?>"> <span class="error">* <?php echo $ulicaErr;?></span> <b>��slo domu: </b>  <input type="number" name="cd" value="<?php echo $cd;?>"> <span class="error">* <?php echo $cdErr;?></span>
                </div>
                <div style="position: relative; top: -30px; left:100px;">
                    <b>Obec/Mesto: </b>  <input type="text" name="obec" value="<?php echo $obec;?>">  <span class="error">* <?php echo $obecErr;?></span> <b>Ps�: </b>  <input type="number" name="psc" value="<?php echo $psc;?>"> <span class="error">* <?php echo $pscErr;?></span>
                </div>

                <div style="position: relative; top: -30px;  left:100px;">
                    <b>�t�t:</b> <input type="text" name="stat" value="<?php echo $stat;?>">  <span class="error">* <?php echo $statErr;?></span>
                    <br>
                </div>
                <div style="position: relative; top: -30px; left:10px">
...
VALUES ('', '', ' ', '', '', '', '','','','','')";
       ...
       if ($_SERVER["REQUEST_METHOD"] == "POST") {
           ...
           if (empty($_POST["menoo"])) {
               ...
               if (!preg_match("/^[a-zA-Z ]*$/",$menoo)) {
                   $menooErr = "Povolen� s� len p�smen� a medzery"; 
               }
           }
           
           if (empty($_POST["priezo"])) {
               $priezoErr = "Priezvisko je potrebn�";
           } else {
               $priezo = test_input($_POST["priezo"]);
               if (!preg_match("/^[a-zA-Z ]*$/",$priezo)) {
                   $priezoErr = "Povolen� s� len p�smen� a medzery"; 
               }
           }

           if (empty($_POST["menoa"])) {
               $menoaErr = "Meno je potrebn�";
           } else {
               $menoa = test_input($_POST["menoa"]);
               if (!preg_match("/^[a-zA-Z ]*$/",$menoa)) {
                   $menoaErr = "Povolen� s� len p�smen� a medzery"; 
               }
           }

           if (empty($_POST["prieza"])) {
               $priezaErr = "Priezvisko je potrebn�";
           } else {
               $prieza = test_input($_POST["prieza"]);
               if (!preg_match("/^[a-zA-Z ]*$/",$prieza)) {
                   $priezaErr = "Povolen� s� len p�smen� a medzery"; 
               }
           }
           if (empty($_POST["ulica"])) {
               ...
               if (!preg_match("/^[a-zA-Z ]*$/",$ulica)) {
                   $ulicaErr = "Povolen� s� len p�smen� a medzery"; 
               }
           }
           if (empty($_POST["cd"])) {
               $cdErr = "��slo je potrebn�";
           } else {
           ...
           if (empty($_POST["obec"])) {
               ...
               if (!preg_match("/^[a-zA-Z ]*$/",$obec)) {
                   $obecErr = "Povolen� s� len p�smen� a medzery"; 
               }
           }

           if (empty($_POST["psc"])) {
               $pscErr = "Ps� je potrebn�";
           } else {
               $psc = test_input($_POST["psc"]);
               if (!preg_match("/^[0-9 ]*$/",$psc)) {
                   $pscErr = "Povolen� s� len ��sla a medzery"; 
               }
           }

           
           if (empty($_POST["stat"])) {
               $statErr = "�t�t je potrebn�";
           } else {
               $stat = test_input($_POST["stat"]);
               if (!preg_match("/^[a-zA-Z ]*$/",$stat)) {
                   $statErr = "Povolen� s� len p�smen� a medzery"; 
               }
           }
           ...
           if (empty($_POST["druh"])) {
               ...
               if (!preg_match("/^[a-zA-Z ]*$/",$druh)) {
                   $druhErr = "Povolen� s� len p�smen� a medzery"; 
               }
           }
           ...
           if (empty($_POST["cena"])) {
               ...
               if (!preg_match("/^[0-9,]*$/",$cena)) {
                   $cenaErr = "Povolen� s� len ��sla a znak ,"; 
               }
           }
       ...
       echo " ";
        ...
        if (!preg_match("/^[a-zA-Z ]*$/",$obec)) {
            $obec = "Povolen� s� len p�smen� a medzery"; 
        } else {echo $obec;}
        echo "<br>";
